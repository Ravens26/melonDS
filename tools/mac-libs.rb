...
def get_rpaths(lib)
  ...
  out = out.split("\n")
  rpaths = out.each_cons(3).map do |line1, line2, line3|
    line1.match(/^ *cmd LC_RPATH$/) ? line3.strip.split(" ")[1] : nil
  end.compact
  rpaths
end
  ...
  if path.match(/@(rpath|loader_path|executable_path)/)
    ...
    when "rpath"
      ...
      rpaths.each do |rpath|
        ...
        if rpath.start_with?("@executable_path")
          relative = rpath.sub(/^@executable_path/, "")
          return File.join("#{$bundle}/Contents/MacOS", relative, file_name), :executable_path
        end
      end

      file = $fallback_rpaths.map { |it| File.join(it, file_name) }.find { |it| File.exist?(it) }
      if file.nil?
...
def install_name_tool(exec, action, path1, path2 = nil)
  ...
  Open3.popen3("install_name_tool", *args, exec) do |stdin, stdout, stderr, thread|
    print stdout.read
    err = stderr.read
    print err unless err.match?("code signature")
  end
end
...
def fixup_libs(prog, orig_path)
  ...
  libs = get_load_libs(prog).map { |it| expand_load_path(orig_path, it) }.reject { |it| system_lib?(it[0]) }
  ...
  libs.each do |lib|
    libpath, libtype = lib
    next if File.basename(libpath) == File.basename(prog)
...
unless File.exist?($bundle) && File.exist?(File.join($build_dir, "CMakeCache.txt"))
  puts "#{$build_dir} doesn't look like a valid build directory"
  exit 1
end

qt_major, qt_dir = File.read(File.join($build_dir, "CMakeCache.txt")).scan(/Qt(.)_DIR:PATH=(.*)/).first
qt_dir = File.absolute_path("#{qt_dir}/../../..")

get_load_libs(executable).each do |lib|
  next if system_lib?(lib)

  path = File.dirname(lib)
  path = path.sub(/\/[^\/]+\.framework.*/, "") if path.match?(".framework")
...
exec_rpaths.reject { |path| path == want_rpath }.each do |path|
